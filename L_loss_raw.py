...
class LPIPSWithDiscriminator(nn.Module):
    def __init__(self, *, logvar_init=0.0,
        ...
        # self.perceptual_loss = torch.FFT().eval()
    ...
    def forward(self, inputs,
        ...
        # nll raw_loss --> weighted_nll_loss
        ...
        avg_loss_ = torch.sum(nll_loss) / (nll_loss.shape[-1] * nll_loss.shape[-2] * nll_loss.shape[-3])
        nll_loss_ = torch.sum(nll_loss) / nll_loss.shape[0]
        kl_loss_ = torch.sum(kl_loss) / kl_loss.shape[0]

        print(f'   >>avg_loss:{avg_loss_}')
        print(f'   >>nll_loss_:{nll_loss_}')
        print(f'   >>weighted_nll_loss_(weighted_nll_loss):{weighted_nll_loss}')
        print(f'   >>kl_loss:{kl_loss_}')
        ...
        if loss_choice == 2:
            ...
            # raw_loss == total_loss == weighted_nll_loss + self.kl_weight * kl_loss + d_weight * disc_factor * g_loss #
            loss = weighted_nll_loss + self.kl_weight * kl_loss + d_weight * disc_factor * g_loss
            print(f'weighted_nll_loss:{weighted_nll_loss}   self.kl_weight * kl_los={self.kl_weight}*{kl_loss}')
